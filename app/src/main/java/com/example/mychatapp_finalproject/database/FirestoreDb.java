...
public class FirestoreDb implements IDatabaseHelper {
    ...
    @Override
    public <T> void getModel(String id, @NonNull Model model, Class<T> clazz, ICallback callback) {
    ...
    @Override
    public void getUserWithSameEmail(String email, @NonNull Model model, ICallback callback) {
    ...
    @Override
    public void getAllMessagesOfChat(ArrayList<String> modelsId, @NonNull Model model, ICallback callback) {
        firestore.collection(model.toString()).whereIn("senderId", modelsId)
                .whereIn("receiverId", modelsId).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                ArrayList<UserMessage> messages = new ArrayList<>();
                for (QueryDocumentSnapshot document : task.getResult()) {
                    messages.add(document.toObject(UserMessage.class));
                }
                Log.d(TAG, "Num of messages: " + messages.size());
